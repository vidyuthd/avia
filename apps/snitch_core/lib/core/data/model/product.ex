defmodule Snitch.Data.Model.Product do
  ...
  def get_rummage_product_list(rummage_opts) do
    opts =
      if rummage_opts do
        rummage_opts |> convert_to_atom_map()
      else
        Map.new()
      end

    {query, rummage} = opts |> Product.rummage()
  ...
  defp to_atom_map(key, map) when is_map(map),
  ...
  defp to_atom_map(k, v) when is_bitstring(v), do: v |> String.to_atom()

  defp to_atom_map(k, v), do: v
